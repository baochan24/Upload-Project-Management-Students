...
{
    public class KhoaForm : Form
    {
        private readonly DataGridView dgvKhoa;
        private readonly TextBox txtMaKhoa;
        private readonly TextBox txtTenKhoa;
        ...
        public KhoaForm()
        {
            ...
            dgvKhoa.SelectionChanged += Dgv_SelectionChanged;

            var panel = new Panel { Dock = DockStyle.Top, Height = 240, Padding = new Padding(10) };

            txtMaKhoa  = CreateLabeledTextBox(panel, "Mã khoa",  10);
            txtTenKhoa = CreateLabeledTextBox(panel, "Tên khoa", 80);
        ...
        {
            try { dgvKhoa.DataSource = KhoaBUS.LoadAll(keyword); }
            catch (Exception ex) { MessageBox.Show(ex.Message, "Lỗi", MessageBoxButtons.OK, MessageBoxIcon.Error); }
        }

        private void Dgv_SelectionChanged(object sender, EventArgs e)
        {
            if (dgvKhoa.CurrentRow?.DataBoundItem is DataRowView row)
            {
                txtMaKhoa.Text  = row["MaKhoa"]?.ToString();
        ...
        {
            var r = KhoaBUS.Add(txtMaKhoa.Text, txtTenKhoa.Text);
            Show(r); if (r.Success) { ClearForm(); LoadData(); }
        }

        private void BtnUpdate_Click(object sender, EventArgs e)
        {
            var r = KhoaBUS.Update(txtMaKhoa.Text, txtTenKhoa.Text);
        ...
        private void BtnDelete_Click(object sender, EventArgs e)
        {
            ...
            var r = KhoaBUS.Delete(txtMaKhoa.Text);
            Show(r); if (r.Success) { ClearForm(); LoadData(); }
        }

        private void ClearForm() { txtMaKhoa.Text = txtTenKhoa.Text = string.Empty; }
